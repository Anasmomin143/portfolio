  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (Array.isArray(parsedData)) {
        projectsData = { projects: parsedData };
      } else if (parsedData.projects && Array.isArray(parsedData.projects)) {
        projectsData = parsedData;
      } else {
        throw new Error('Invalid format. Expected an array of projects or { projects: [...] }');
      }

      // Add skip duplicates flag
      projectsData.skipDuplicates = skipDuplicates;
          ...
          <div className="max-w-5xl">
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={COMMON_INLINE_STYLES.textMuted}>
                      Don&apos;t import projects with IDs that already exist
                    </p>
                  </div>
                </label>
              </div>
                ...
                {result.success && result.imported > 0 && (
                  <div className="mt-4 p-4 rounded-lg flex items-center gap-3" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)' }}>
                    ...
                    <p className="text-sm" style={{ color: '#10b981' }}>
                      All projects imported successfully! Redirecting to projects list...
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-xl p-6" style={{ background: THEME_GRADIENTS.card, border: '1px solid var(--card-border)' }}>
              ...
              <p className="text-sm mb-4" style={COMMON_INLINE_STYLES.textMuted}>
                Your JSON should follow this structure. You can import a single project or multiple projects at once.
              </p>
